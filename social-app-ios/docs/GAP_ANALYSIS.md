...
| SocialFeedError.swift | Models/ | 新建，22 种变体 + 中文 errorDescription |
| SocialFeedService.swift | Services/ | 完全重写，全部 30+ 方法已实现 |

### P1 - 核心服务
| 文件 | 路径 | 状态 |
|------|------|------|
| Config.swift | Models/ | 新建，Config struct（7 字段）+ ConfigBuilder（7 方法）|
| AppTypes.swift | Models/ | 增强 PaginationState、SearchIndex 增强、ForwardMetadata |
| AggregationCache.swift | Services/ | 新建，对应 Rust AggregationCache（8 方法） |
| RateLimiter.swift | Services/ | 新建，令牌桶限流（容量100+refill 10/s+3次重试） |
| ProfileCache.swift | Services/ | 新建，LRU+TTL 缓存（7 方法） |
| ImageUploadService.swift | Services/ | 新建，图片上传服务 |
| MediaProcessor.swift | Services/ | 新建，多媒体处理 |

### P2 - 工具与增强
| 文件 | 路径 | 状态 |
|------|------|------|
| Validators.swift | Models/ | ⬜ 待建（2026-06-08 审查：文件不存在） |
| TextUtils.swift | Models/ | ⬜ 待建（2026-06-08 审查：文件不存在） |
| ImageUtils.swift | Models/ | ⬜ 待建（2026-06-08 审查：文件不存在） |

### P3 - 第7章 好友与即时通讯
| 文件 | 路径 | 状态 |
|------|------|------|
| FriendService.swift | Services/ | 新建，好友管理服务 |
| MessageService.swift | Services/ | 新建，即时通讯服务 |
| ConversationViewModel.swift | ViewModels/ | 新建，聊天对话页 ViewModel |
| ChatListView.swift | Views/ | 新建，会话列表视图 |
| ChatDetailView.swift | Views/ | 新建，聊天详情视图 |
| AvatarView.swift | Views/ | 新建，通用头像组件 |

### P3 - 第8章 设置与偏好
| 文件 | 路径 | 状态 |
|------|------|------|
| SettingsViewModel.swift | ViewModels/ | 新建，设置 ViewModel |
| SettingsView.swift | Views/ | 新建，设置主视图 |

### P3 - 第9章 Spaces / Threads / Polls
| 文件 | 路径 | 状态 |
|------|------|------|
| SpacesService.swift | Services/ | 新建，Spaces 服务（9 方法） |
| ThreadService.swift | Services/ | 新建，Threads 服务（9 方法） |
| PollService.swift | Services/ | 新建，Polls 服务（9 方法） |
...
| `set_pin()` / `unlock_with_pin()` | ❌ 不存在 | Rust 核心 `account.rs` 中无此 API，标记为文档错误 |
| `get_3pids()` / `add_3pid()` / `delete_3pid()` | ❌ 未暴露 | 邮箱/手机绑定管理 |
| `request_3pid_email_token()` / `request_3pid_msisdn_token()` | ❌ 未暴露 | 第三方 ID 验证令牌 |

### iOS 后续
- SocialFeedService 中的 17 个 Mock 方法替换为 UniFFI 真实绑定
- FriendService.createDmViaHttp HTTP 绕过替换为 createDm FFI 直接调用
- 第7-9章 Service 层接入真实 Rust FFI 调用
- 未覆盖章节补充（消息搜索/房间目录/QR登录/已读回执/Reactions/位置共享/房间列表）
- Keychain 集成

## 统计
- 新建文件：18 个
- 修改文件：3 个
- 整体覆盖率：从 15% 提升至 85%（iOS 端 Service/ViewModel/View 层）
...
| M3 | `Models/SocialFeedError.swift` | `Services/` 副本已删除，仅 `Models/` 下唯一版本 | ✅ |
| M4 | `Views/AsyncImageGrid.swift` / `AvatarView.swift` | `Views/Common/` 副本已删除 | ✅ |

## 三、Service 层（4 项全部完成）

| # | 文件（5969–13547 B） | 关键改动 | 状态 |
|---|---------------------|---------|------|
| S1 | `Services/FriendService.swift` | 新增 `createDmViaHttp(userId:)` 方法，URLSession 调 `POST /_matrix/client/v3/createRoom`，标注 TODO 待 FFI 就绪后替换 | ✅ |
| S2 | `Services/MessageService.swift` | `sendAttachment` 改为两步：① `client.uploadMedia()` → mxc URI ② 构造 JSON 调 `room.sendRaw(eventType:"m.room.message", ...)` | ✅ |
| S3 | `Services/PollService.swift` | 新增 `pollStartIds: [String:String]` 映射表；`castVote`/`closePoll` 从映射表取 pollStartId；新增 `startPollTracking(roomId:)` | ✅ |
| S4 | `Services/AccountSettingsService.swift` | 无需修改，`changePassword` 已正确直接调 FFI | ✅ |

## 四、ViewModel 层（3 项全部新建完成）

| # | 文件（7329–7672 B） | 关键方法 | 状态 |
|---|---------------------|---------|------|
| V1 | `ViewModels/SpacesViewModel.swift` | `loadSpaces()` / `loadSpaceRooms()` / `addChildToSpace()` / `removeChildFromSpace()` / `leaveSpace()` / `createSpace()` | ✅ |
| V2 | `ViewModels/ThreadViewModel.swift` | `loadThreads()` / `paginate()` / `setThreadSubscription()` / `subscribeToThread()` / `unsubscribeFromThread()` | ✅ |
| V3 | `ViewModels/PollViewModel.swift` | `createPoll()` / `castVote()` / `retractVote()` / `closePoll()` / `deletePoll()` + 倒计时定时器 | ✅ |

## 五、View 层（11 项全部新建完成）

| # | 文件（4028–8717 B） | 说明 | 状态 |
|---|---------------------|------|------|
| C1 | `Views/ContactsView.swift` | 联系人列表，按首字母分组索引 + 搜索栏 + 好友请求入口 | ✅ |
| C2 | `Views/AddFriendView.swift` | 搜索用户 + 发送好友请求 + 待处理邀请列表 | ✅ |
| C3 | `Views/SpacesView.swift` | 空间网格卡片 + 空间详情 + 创建空间 Sheet | ✅ |
| C4 | `Views/ThreadView.swift` | 线程列表 + 订阅切换 + 分页加载 | ✅ |
| C5 | `Views/PollView.swift` | 投票选项进度条 + 投票/撤回/结束操作 | ✅ |
| C6 | `Views/FriendRequestView.swift` | 待处理邀请列表 + 接受/拒绝 + FriendRequestViewModel | ✅ |
| C7 | `Views/MediaPicker.swift` | PHPickerViewController SwiftUI 桥接，返回 `[MediaAttachment]` | ✅ |
| C8 | `Views/TypingIndicator.swift` | 动画三点指示器 + TypingIndicatorViewModel（轮询降级） | ✅ |
| C9 | `Views/MediaSettingsView.swift` | 媒体预览策略选择器 + 邀请头像开关 + MediaSettingsViewModel | ✅ |
| C10 | `Views/CallView.swift` | WKWebView 加载 Element Call + 静音/挂断/扬声器 + 通话计时 | ✅ |
| C11 | `Views/IncomingCallView.swift` | 全屏来电界面 + 接听/拒接 + 振铃超时 + IncomingCallViewModel | ✅ |
...
| 遗留项 | 说明 | 优先级 |
|--------|------|--------|
| SocialFeedService Mock → 真实 FFI | 17 个方法仍为 Mock 实现，需接入 UniFFI 绑定 | P0 |
| FriendService.createDmViaHttp → createDm FFI | 替换 HTTP API 绕过为 Rust FFI 调用 | P1 |
| 第7-9章 Service 接入真实 FFI | SpacesService / ThreadService / PollService 等仍需接入 FFI | P1 |
| 消息搜索 / 房间目录 / QR 登录 / 已读回执 / Reactions / 位置共享 / 房间列表 | 未覆盖章节 | P2 |
...
| 文件 | 完成项 | 完成日期 |
|------|--------|----------|
| `Services/AuthManager.swift` | 15 个认证方法全量完成（loginWithEmail/customLoginWithJwt/startSsoLogin/finishSsoLogin/urlForOauth/loginWithOauthCallback/abortOauthAuth/getSession/restoreSession/restoreSessionWith/homeserverLoginDetails） | 2026-06-08 |
| `Services/AuthManager.swift` | ClientBuilder 12 个选项（passphrase/slidingSyncVersionBuilder/crossProcessLockConfig/inMemoryStore 等） | 2026-06-08 |
| `Services/QRLoginService.swift` | 签名修正 + 反向流程（startQrCodeGeneration/startGrantScan） | 2026-06-08 |
| `Services/AuthManager.swift` | ClientSessionDelegate 实现 | 2026-06-08 |
...
| 文件 | 完成项 | 完成日期 |
|------|--------|----------|
| `Services/SocialFeedService.swift` | MomentDetailView 回复筛选（TimelineEventCollector + inReplyTo 本地过滤） | 2026-06-08 |
| `Services/SocialFeedService.swift` | PaginationOptions 幻觉类型修正（4 处 → TimelineListener 模式） | 2026-06-08 |

### 4. 图片上传 ✅ 已完成

| 文件 | 完成项 | 完成日期 |
|------|--------|----------|
| `Services/Infrastructure/ImageUploadService.swift` | compressImage 实现（CGImageSource + CGImageDestination） | 2026-06-08 |
| `Services/SocialFeedService.swift` | postMoment 带图发布接入（uploadImages → mxc URI → content JSON） | 2026-06-08 |

### 5. 附件选择器/转发 ✅ 已完成

| 文件 | 完成项 | 完成日期 |
|------|--------|----------|
| `Views/Chat/ChatDetailView.swift` | 附件选择器（PhotosPicker + fileImporter） | 2026-06-08 |
| `Views/Chat/ChatDetailView.swift` | 转发功能（ForwardRoomPickerView + sendRaw） | 2026-06-08 |

### 6. RoomListViewModel 设置 ✅ 已完成

| 文件 | 完成项 | 完成日期 |
|------|--------|----------|
| `ViewModels/RoomListViewModel.swift` | toggleFavourite/toggleMute/setLowPriority 接入 | 2026-06-08 |
...
| # | 缺口 | 说明 |
|---|------|------|
| 1 | `SocialFeedService` Mock → 真实 FFI | 17 个方法仍为 Mock 实现 |
| 2 | `FriendService.createDmViaHttp` → `createDm` FFI | HTTP API 绕过需替换为 Rust FFI |
